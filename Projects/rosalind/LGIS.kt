...
object LGIS {
    private fun dp(s: IntArray, d: Int): List<Integer> {
        val n = s.size
        val len = IntArray(n)
        val prv = IntArray(n)
        Arrays.fill(len, 1)
        ...
        for (i in 1 until n)
            for (j in 0 until i)
                if ((if (s[j] < s[i]) 1 - d else d) == 1 && len[i] < len[j] + 1) {
                    len[i] = len[j] + 1
                    prv[i] = j
                }
        var m = IntStream.range(0, n).boxed().max(Comparator.comparingInt({ i -> len[i] })).get()
        val dp = ArrayList()
        do {
            dp.add(s[m])
        } while ((m = prv[m]) != -1)
