...
inline fun <reified T> readValue(fd: RandomAccessFile): T {
    val size = CLASS2SIZE.getValue(T::class)
...
@ExperimentalUnsignedTypes
fun readEntries(fd: RandomAccessFile): List<ImageEntry> {
    ...
    do {
        ...
        repeat(entryNum) {
            ...
            if (count * TYPE2SIZE[type] > 4)
                fd.seek(readValue<UInt>(fd).toLong())
            val values = readValues(fd, type, count)
            if (tag == 273)
                imageEntry = ImageEntry(Entry(pointer, tag, values))
            else if (tag == 279) {
                imageEntry!!.size = Entry(pointer, tag, values)
...
@ExperimentalUnsignedTypes
fun readImageEntry2Buffer(fd: RandomAccessFile, imageEntry: ImageEntry): ByteArray {
